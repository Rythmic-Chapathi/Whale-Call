    ...
    <div className="grid gap-8 lg:grid-cols-[.68fr_1.32fr]">
      ...
      <form onSubmit={submit} className="rounded-[32px] border border-amber-900/10 bg-card p-6 shadow-lg sm:p-9" data-testid="form-supply-order">
        ...
        {step === 2 && <>
          ...
          <div className="mt-6"><CaribbeanMap islands={islands ?? []} boats={[]} pickupId={depots?.[0]?.islandId} destinationId={coordinates ? undefined : islandId} targetPosition={coordinates ? position : undefined} className="min-h-[240px]" /></div>
...
export function SupplyTrackingPage() {
  ...
  return <main className="mx-auto max-w-[1120px] px-5 py-10 lg:px-8 lg:py-14">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <div><CaribbeanMap islands={islands ?? []} boats={order.boat ? [order.boat] : []} pickupId={depotMap.get(order.lines[0]?.depotId)?.islandId} destinationId={order.destinationIslandId ?? undefined} targetPosition={order.destinationIslandId ? undefined : order.destinationPosition} className="min-h-[430px]" /></div>
