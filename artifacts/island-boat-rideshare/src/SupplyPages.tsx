...
export function SupplyTrackingPage() {
  ...
  return <main className="mx-auto max-w-[1120px] px-5 py-10 lg:px-8 lg:py-14">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[1.4fr_.6fr]">
      <div><CaribbeanMap islands={islands ?? []} boats={order.boat ? [order.boat] : []} pickupId={depotMap.get(order.lines[0]?.depotId)?.islandId} destinationId={order.destinationIslandId ?? undefined} targetPosition={order.destinationIslandId ? undefined : order.destinationPosition} className="min-h-[480px] sm:min-h-[580px]" /></div>
