...
function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className="focus-ring flex items-center gap-3" data-testid="link-logo">
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[14px] bg-secondary text-sidebar">
        <Anchor size={22} strokeWidth={2.5} />
      </span>
      {!compact && <span className="font-display text-[21px] font-semibold tracking-[-.03em] text-sidebar-foreground">Island Boat</span>}
    </Link>
  );
}
...
function Button({ children, kind = 'primary', className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { kind?: 'primary' | 'secondary' | 'quiet' | 'danger' }) {
  ...
  return <button {...props} className={`focus-ring inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-bold transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${styles[kind]} ${className}`} />;
}
...
function Footer() {
  ...
  return <footer className="border-t border-sidebar-border bg-sidebar px-5 py-12 text-sidebar-foreground lg:px-8"><div className="mx-auto grid max-w-[1240px] gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
    ...
    <div><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-secondary">Island office</p><p className="mt-4 text-sm leading-6 text-sidebar-foreground/70">VHF channel 16<br />Daily, first light to last launch<br />hello@islandboat.local</p></div>
  </div></footer>;
}
  ...
  return <AppShell><main>
    <section className="relative overflow-hidden bg-sidebar px-5 pb-24 pt-16 text-sidebar-foreground lg:px-8 lg:pb-32 lg:pt-24">
      ...
      <div className="relative mx-auto grid max-w-[1240px] items-end gap-14 lg:grid-cols-[1.02fr_.98fr]">
        <div className="rise-in"><p className="font-mono-ui text-[11px] uppercase tracking-[.22em] text-secondary">Island Boat · local passage, done right</p><h1 className="mt-7 max-w-2xl font-display text-6xl font-semibold leading-[.92] tracking-[-.055em] sm:text-8xl">Get there by<br /><span className="text-secondary">water.</span></h1><p className="mt-8 max-w-md text-base leading-7 text-sidebar-foreground/70">A ride across the bay, called by people who know every shoal, dock, and changing tide.</p><div className="mt-9 flex flex-wrap gap-3"><Link href="/book" className="focus-ring inline-flex items-center gap-2 rounded-full bg-secondary px-6 py-3.5 text-sm font-extrabold text-sidebar transition-transform hover:-translate-y-1" data-testid="link-hero-book">Plan a crossing <ArrowRight size={17} /></Link><Link href="/fleet" className="focus-ring inline-flex items-center gap-2 rounded-full border border-sidebar-foreground/20 px-6 py-3.5 text-sm font-bold text-sidebar-foreground hover:bg-sidebar-foreground/10" data-testid="link-hero-fleet">Meet the fleet</Link></div></div>
...
function HomeRoute() {
  ...
  return signedIn ? <Redirect to="/book" /> : <Landing />;
}
